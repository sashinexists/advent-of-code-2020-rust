...
pub fn run() {
    ...
    println!(
        "The fixed report comes to {}",
        fix_expense_report(&expense_report)
    );
}
...
fn fix_expense_report(expense_report: &Vec<i32>) -> i32 {
    for i in 0..expense_report.len() {
        for j in 0..expense_report.len() {
            for k in 0..expense_report.len() {
                if i!=j && i!=k && j!=k && expense_report[i] + expense_report[j] + expense_report[k] == 2020 {
